...
// remember to return write size or error code
long write(trapframe_t *tp, int fd, const void *buf, unsigned long count)
{
    if (curr_thread->file_descriptors_table[fd])
    {
        tp->x0 = vfs_write(curr_thread->file_descriptors_table[fd], buf, count);
        return tp->x0;
    }
...
long read(trapframe_t *tp, int fd, void *buf, unsigned long count)
{
    if (curr_thread->file_descriptors_table[fd])
    {
        tp->x0 = vfs_read(curr_thread->file_descriptors_table[fd], buf, count);
        return tp->x0;
    }
...
int mount(trapframe_t *tp, const char *src, const char *target, const char *filesystem, unsigned long flags, const void *data)
{
    ...
    get_absolute_path(abs_path, curr_thread->curr_working_dir);
    tp->x0 = vfs_mount(abs_path, filesystem);
    return tp->x0;
}
