...
void svc_router(trapframe_t *tp)
{
    ...
    else if (syscall_no == 11) { open(tp, (char*)tp->x0, tp->x1);                                                       }
    else if (syscall_no == 12) { close(tp, tp->x0);                                                                     }
    else if (syscall_no == 13) { write(tp, tp->x0, (char *)tp->x1, tp->x2);                                             }
    else if (syscall_no == 14) { read(tp, tp->x0, (char *)tp->x1, tp->x2);                                              }
    else if (syscall_no == 15) { mkdir(tp, (char *)tp->x0, tp->x1);                                                     }
    else if (syscall_no == 16) { mount(tp, (char *)tp->x0, (char *)tp->x1, (char *)tp->x2, tp->x3, (void*)tp->x4);      }
    else if (syscall_no == 17) { chdir(tp, (char *)tp->x0);                                                             }
    else if (syscall_no == 18) { lseek64(tp, tp->x0, tp->x1, tp->x2);                                               }
    else if (syscall_no == 19) { ioctl(tp, tp->x0, tp->x1, (void*)tp->x2);                                          }
