#include "vfs.h"
#include "dev_framebuffer.h"
#include "uart1.h"
#include "memory.h"
#include "mbox.h"
#include "string.h"
#include "exception.h"

#define MBOX_CH_PROP 8

// The following code is for mailbox initialize used in lab7.
unsigned int width, height, pitch, isrgb; /* dimensions and channel order */
unsigned char *lfb;                       /* raw frame buffer address */

struct file_operations dev_framebuffer_operations = {dev_framebuffer_write, (void *)dev_framebuffer_op_deny, dev_framebuffer_open, dev_framebuffer_close, dev_framebuffer_lseek64, (void *)dev_framebuffer_op_deny};

//https://github.com/raspberrypi/firmware/wiki/Mailbox-property-interface#frame-buffer
int init_dev_framebuffer()
{
    // The following code is for mailbox initialize used in lab7.
    m[0] = 35 * 4;
    m[1] = MBOX_TAG_REQUEST_CODE;

    //https://github.com/raspberrypi/firmware/wiki/Mailbox-property-interface#set-physical-display-widthheight
    //  Set physical (display) width/height
    m[2] = 0x48003; // set phy wh
    m[3] = 8;
    m[4] = 8;
    m[5] = 1024; // FrameBufferInfo.width
    m[6] = 768;  // FrameBufferInfo.height

    //https://github.com/raspberrypi/firmware/wiki/Mailbox-property-interface#set-virtual-buffer-widthheight
    //  Set virtual (buffer) width/height
    m[7] = 0x48004; // set virt wh
    m[8] = 8;
    m[9] = 8;
    m[10] = 1024; // FrameBufferInfo.virtual_width
    m[11] = 768;  // FrameBufferInfo.virtual_height

    //https://github.com/raspberrypi/firmware/wiki/Mailbox-property-interface#set-virtual-offset
    //  Set virtual offset
    m[12] = 0x48009; // set virt offset
    m[13] = 8;
    m[14] = 8;
    m[15] = 0; // FrameBufferInfo.x_offset
    m[16] = 0; // FrameBufferInfo.y.offset

    //https://github.com/raspberrypi/firmware/wiki/Mailbox-property-interface#set-depth
    //  Set depth
    m[17] = 0x48005; // set depth
    m[18] = 4;
    m[19] = 4;
    m[20] = 32; // FrameBufferInfo.depth

    //https://github.com/raspberrypi/firmware/wiki/Mailbox-property-interface#set-pixel-order
    //  Set pixel order
    m[21] = 0x48006; // set pixel order
    m[22] = 4;
    m[23] = 4;
    m[24] = 1; // RGB, not BGR preferably

    //https://github.com/raspberrypi/firmware/wiki/Mailbox-property-interface#allocate-buffer
    //  Allocate buffer
    m[25] = 0x40001; // get framebuffer, gets alignment on request
    m[26] = 8;
    m[27] = 8;
    m[28] = 4096; // FrameBufferInfo.pointer;   frame buffer base address in bytes
    m[29] = 0;    // FrameBufferInfo.size;      frame buffer size in bytes

    //https://github.com/raspberrypi/firmware/wiki/Mailbox-property-interface#get-pitch
    //  Get pitch
    m[30] = 0x40008; // get pitch
    m[31] = 4;
    m[32] = 4;
    m[33] = 0; // FrameBufferInfo.pitch

    m[34] = MBOX_TAG_LAST_BYTE;

    // this might not return exactly what we asked for, could be
    // the closest supported resolution instead
    if (mbox_call(MBOX_TAGS_ARM_TO_VC, (unsigned int)((unsigned long)&m)) && m[20] == 32 && m[28] != 0)
    {
        m[28] &= 0x3FFFFFFF; // convert GPU address to ARM address
        width = m[5];        // get actual physical width
        height = m[6];       // get actual physical height
        pitch = m[33];       // get number of bytes per line
        isrgb = m[24];       // get the actual channel order
        lfb = PHYS_TO_VIRT((void *)((unsigned long)m[28])); // raw frame buffer address
    }
    else
    {
        uart_puts("Unable to set screen resolution to 1024x768x32\n");
    }

    return register_dev(&dev_framebuffer_operations);
}

int dev_framebuffer_write(struct file *file, const void *buf, size_t len)
{
    lock();
    if (len + file->f_pos > pitch * height)
    {
        len = pitch * height - file->f_pos;
    }
    memcpy(lfb + file->f_pos, buf, len);
    file->f_pos += len;
    unlock();
    return len;
}

int dev_framebuffer_open(struct vnode *file_node, struct file **target)
{
    (*target)->f_pos = 0;
    (*target)->vnode = file_node;
    (*target)->f_ops = &dev_framebuffer_operations;
    return 0;
}

int dev_framebuffer_close(struct file *file)
{
    kfree(file);
    return 0;
}

long dev_framebuffer_lseek64(struct file *file, long offset, int whence)
{
    if (whence == SEEK_SET)
    {
        file->f_pos = offset;
        return file->f_pos;
    }
    return -1;
}

int dev_framebuffer_op_deny()
{
    return -1;
}