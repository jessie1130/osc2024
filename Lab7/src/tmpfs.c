...
//you should mount tmpfs at rootfs, at this point, 
//lookup might not be available, you can call setup_mount directly to mount it.
...
struct vnode *tmpfs_create_vnode(struct mount *_mount, enum fsnode_type type)
{
    ...
    v->f_ops = &tmpfs_file_operations;  //write, read, open, close, lseek64, getsize
    v->v_ops = &tmpfs_vnode_operations; //create, lookup, mkdir
    v->mount = 0;
...
int tmpfs_lookup(struct vnode *dir_node, struct vnode **target, const char *component_name)
{
    ...
    //uart_sendline("tmpfs_lookup: can't find\n");
    return -1;
}
...
int tmpfs_create(struct vnode *dir_node, struct vnode **target, const char *component_name)
{
    ...
    if (inode->type != dir_t)
    {
        uart_sendline("tmpfs_create: not dit_t\n");
        return -1;
    }
    ...
    for (; child_idx <= MAX_DIR_ENTRY; child_idx++)
    {
        ...
        {
            uart_sendline("tmpfs_create: file already exist\n");
            return -1;
        }
    }
    ...
    {
        uart_sendline("tmpfs_create: directory entry is full\n");
		return -1;
    }

    if (strlen(component_name) > MAX_FILE_NAME)
    {
        uart_sendline("tmpfs_create: file name is too long\n");
        return -1;
    }
...
int tmpfs_mkdir(struct vnode *dir_node, struct vnode **target, const char *component_name)
{
    ...
    if (inode->type != dir_t)
    {
        uart_sendline("tmpfs_mkdir: type is not directory\n");
        return -1;
    }
    ...
    for (; child_idx <= MAX_DIR_ENTRY; child_idx++)
    {
        ...
        if (strcmp(child_inode->name,component_name)==0)
        {
            uart_sendline("tmpfs_mkdir: directory already exist\n");
            return -1;
        }
    }
    ...
    if (child_idx > MAX_DIR_ENTRY)
    {
        uart_sendline("tmpfs_mkdir: directory entry is full\n");
		return -1;
    }

    if (strlen(component_name) > MAX_FILE_NAME)
    {
        uart_sendline("tmpfs_mkdir: file name is too long\n");
        return -1;
    }
